...
export default function DoctorQueuePage() {
  ...
  const [queue, setQueue] = useState<QueueEncounter[]>([]);
  ...
  const loadQueue = async () => {
    setLoading(true);
    try {
      let url = "/api/opd/queue?";
      if (selectedDoctorId) url += `&doctorId=${selectedDoctorId}`;
      if (selectedDeptId) url += `&departmentId=${selectedDeptId}`;

      const res = await apiClient<QueueEncounter[]>(url);
      setQueue(res);
    } catch {
      toast.error("Failed to load doctor queues.");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    async function loadFormMetadata() {
      try {
        ...
        // Suppress initial metadata errors
      }
    }
    loadFormMetadata();
  }, []);
  ...
  useEffect(() => {
    ...
    async function loadDetails() {
      ...
      try {
        ...
        toast.error("Failed to load patient card details.");
  ...
  const handlePrintSlip = async (item: QueueEncounter) => {
    try {
      ...
      try {
          ...
          body: JSON.stringify({
            ...
            printData: {
              ...
              content: {
                ...
                "Address": (item.patient as any).address || "--",
              },
        ...
        console.error("Failed to compile consult slip from queue list", err);
        setConsultHtml(null);
      }
    } catch {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-100 tracking-tight">Physician Consultation Queue</h1>
            <p className="text-xs text-slate-400">Track active daily token lineups and patient assignments</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2 shrink-0">
          <button
            onClick={loadQueue}
            className="p-3 bg-slate-900 border border-slate-800 hover:border-slate-750 text-slate-400 hover:text-white rounded-xl transition-all cursor-pointer"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search patient name, UHID, OPD ID..."
            />
          </div>
          ...
          <select
            ...
            className="w-full bg-slate-950 border border-slate-800 hover:border-slate-700 text-slate-100 text-xs rounded-xl px-4 py-2.5 outline-none"
          >
      ...
      {loading ? (
        <div className="h-64 flex items-center justify-center text-zinc-400 text-sm font-mono">
          ...
          <span>Polling Doctor Token Queues...</span>
        </div>
      ) : (
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="border-b border-slate-800/80 bg-slate-950/20 text-slate-450 font-semibold select-none">
                  ...
                  <th className="p-4">Clinical Department</th>
              ...
              <tbody className="divide-y divide-slate-800/50 text-zinc-300">
                {filteredQueue.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-800/20 transition-colors">
                    <td className="p-4 font-bold text-base text-emerald-400 font-mono select-none">{item.tokenNumber}</td>
                    <td className="p-4 font-mono font-bold text-slate-205">{item.opdId}</td>
                    <td className="p-4 font-semibold text-slate-100">{item.patient.name}</td>
                    <td className="p-4 font-mono">{item.patient.uhid}</td>
                    <td className="p-4 font-medium">{item.doctor.employee.name}</td>
                    ...
                    <td className="p-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <button
                          onClick={() => setCancelEncounterId(item.id)}
                ...
                {filteredQueue.length === 0 && (
                  <tr>
                    <td colSpan={8} className="text-center p-12 text-zinc-550 font-mono text-xs">
                      No active patient consultations waiting in queue.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col h-[90vh] animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex-1 bg-slate-900/40 p-4 overflow-auto flex justify-center items-center">
              ...
              >
                <iframe
                  ref={iframeRef}
                  title="OPD Queue Slip Print Preview"
