 ...
 * Returns a paginated summary list of laboratory orders.
 * Revision history is excluded to optimize search performance.
 */
export const GET = wrapAuthRoute(async (req: NextRequest) => {
  ...
  if (status.trim()) {
    where.status = status as LaboratoryStatus;
  }
  if (doctorId.trim()) {
  ...
  const total = await prisma.labTestOrder.count({ where });
