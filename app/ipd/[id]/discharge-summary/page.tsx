  ...
  useEffect(() => {
    async function loadPrint() {
      try {
        ...
        setHtml("<p style='color:red;font-family:sans-serif;padding:20px;'>Failed to render Discharge Summary print layout.</p>");
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center space-y-2 bg-white text-slate-800">
        ...
        <span className="text-xs font-mono">Compiling Discharge Summary A4 Slip...</span>
      </div>
    );
  }
