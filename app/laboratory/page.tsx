...
export default function LaboratoryDirectoryPage() {
  ...
  const loadOrders = async () => {
    setLoading(true);
    try {
      let url = `/api/laboratory/orders?page=${page}&limit=10`;
      ...
      const msg = err instanceof Error ? err.message : "Failed to load laboratory orders.";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleCollectSampleDirect = async (orderId: string) => {
    setActionSavingId(orderId);
    try {
  ...
  const columns = useMemo<ColumnDef<LabOrderSummary>[]>(
    () => [
      {
        header: "Order ID",
        accessorKey: "id",
      ...
      {
        header: "Test Name",
        accessorKey: "testCatalog.name",
        cell: (info) => <span className="text-emerald-400 font-medium">{info.getValue() as string}</span>,
      },
      {
        header: "Category",
        accessorKey: "testCatalog.category",
        cell: (info) => <span className="text-zinc-500 font-mono text-[10px]">{info.getValue() as string}</span>,
      },
      {
        header: "Doctor",
      ...
      },
      {
        ...
        cell: (info) => {
          ...
          if (status === "COMPLETED") badgeClass = "bg-emerald-950/20 text-emerald-450 border border-emerald-900/20";
          if (status === "SAMPLE_COLLECTED") badgeClass = "bg-purple-950/20 text-purple-400 border border-purple-900/20";
      ...
      },
      {
        ...
        cell: (info) => {
          ...
          const isSaving = actionSavingId === row.id;
          return (
            <div className="flex items-center space-x-2">
              <Link
                href={`/laboratory/${row.id}`}
                className="p-1.5 text-slate-400 hover:text-emerald-450 hover:bg-slate-800 rounded-xl transition-all cursor-pointer"
                title="Open Test Report Details"
              >
                <Eye size={14} />
              </Link>
              {row.status === "SCHEDULED" && (
                <button
                  onClick={() => handleCollectSampleDirect(row.id)}
                  disabled={isSaving}
                  className="px-2 py-1 bg-purple-950 border border-purple-800 hover:border-purple-700 text-purple-400 hover:text-purple-300 rounded-lg text-[10px] transition-all flex items-center space-x-1 cursor-pointer disabled:opacity-50 font-bold"
                >
                  {isSaving ? (
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          <h1 className="text-xl font-bold text-slate-100 tracking-tight flex items-center">
            <Beaker size={20} className="mr-2 text-emerald-400" />
            <span>Clinical Laboratory Registry</span>
          </h1>
          <p className="text-xs text-slate-400">Collect samples, record diagnostic test parameters, and print reports</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900/20 border border-slate-800 p-4 rounded-2xl space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-semibold text-slate-400">Lab Status</label>
            <select
              value={statusFilter}
      ...
      {loading ? (
        <div className="h-64 flex items-center justify-center text-zinc-400 text-sm font-mono">
          ...
          <span>Syncing Laboratory Lineup...</span>
        </div>
      ) : (
